...
## GitHub Copilot 高阶工作流

长会话优化方法在 Copilot Chat 中同样成立，但更依赖严格的上下文管理与阶段化执行。

- 将会话拆分为探索、规划、实现、验证、总结五个阶段。
- 先给出精确文件范围，再按需扩展上下文，避免无效上下文膨胀。
- 通过简洁进度记录与下一步清单保持跨会话连续性。
- 使用专用智能体处理边界清晰的子任务，降低上下文串扰。
- 将 lint、测试、类型检查前置，减少末尾集中返工。
- 为高频任务沉淀稳定提示模板，提升多会话一致性。
- 采用小批量 diff 审查，降低回滚成本并提升交付确定性。
